...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        Board_LED_PC =   (Board_LED_PC & \
                                (uint32)(~(uint32)(Board_LED_DRIVE_MODE_IND_MASK << (Board_LED_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (Board_LED_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(Board_LED_DR & (uint8)(~Board_LED_MASK));
    drVal = (drVal | ((uint8)(value << Board_LED_SHIFT) & Board_LED_MASK));
    Board_LED_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(Board_LED__0__SHIFT, mode);
}
...
{
    return (uint8)((Board_LED_PS & Board_LED_MASK) >> Board_LED_SHIFT);
}
...
{
    return (uint8)((Board_LED_DR & Board_LED_MASK) >> Board_LED_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(Board_LED_INTSTAT & Board_LED_MASK);
		Board_LED_INTSTAT = maskedStatus;
        return maskedStatus >> Board_LED_SHIFT;
    }
