...
void LCD_Char_LoadCustomFonts(uint8 const customData[]) 
{
...
#if (LCD_Char_CUSTOM_CHAR_SET == LCD_Char_HORIZONTAL_BG)

    /*******************************************************************************
    * Function Name: LCD_Char_DrawHorizontalBG
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        fullChars = value / LCD_Char_CHARACTER_WIDTH;

        /* Number of remaining pixels to draw */
        remainingPixels = value % LCD_Char_CHARACTER_WIDTH;
        ...
        LCD_Char_Position(row, column);

        /* Write full characters */
        for(count8 = 0u; count8 < fullChars; count8++)
        {
            LCD_Char_WriteData(LCD_Char_CUSTOM_5);
        }

        if(fullChars < maxCharacters)
        {
            /* Write remaining pixels */
            LCD_Char_WriteData(remainingPixels);
            ...
            if(fullChars < (maxCharacters - 1u))
            {
                ...
                {
                    LCD_Char_WriteData(LCD_Char_CUSTOM_0);
                }
            }
        }
    }

#endif /* LCD_Char_CUSTOM_CHAR_SET == LCD_Char_HORIZONTAL_BG */


#if (LCD_Char_CUSTOM_CHAR_SET == LCD_Char_VERTICAL_BG)

    /*******************************************************************************
    *  Function Name: LCD_Char_DrawVerticalBG
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        /* Number of full characters to draw */
        fullChars = value / LCD_Char_CHARACTER_HEIGHT;

        /* Number of remaining pixels to draw */
        remainingPixels = value % LCD_Char_CHARACTER_HEIGHT;
        ...
        {
            LCD_Char_WriteData(LCD_Char_CUSTOM_7);
            ...
            {
                LCD_Char_Position(row - count8, column);
            }
            else
            {
                break;
        ...
        if(((((int8) row) - ((int8) count8)) >= 0) && (fullChars < maxCharacters))
        {
            ...
            {
                LCD_Char_WriteData((uint8) ' ');
            }
            else
            {
                LCD_Char_WriteData(remainingPixels - 1u);
            }
            ...
            if(currentRow >= 0)
            {
                ...
                for(count8 = 0u; count8 < (maxCharacters - fullChars - 1u); count8++)
                {
                    LCD_Char_Position((uint8)currentRow, column);
                    LCD_Char_WriteData((uint8) ' ');
                    currentRow --;
                }
            }
